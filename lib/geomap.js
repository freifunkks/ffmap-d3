define("geomap", [
  ...
  function init() {
    ...
    map.addLayer(new L.TileLayer("http://{s}.tile.osm.org/{z}/{x}/{y}.png", {
      subdomains: "abc",
      type: "osm",
    ...
    g.append("g").attr("class", "backbones")
  ...
  function update(data) {
    ...
    var links = data.links.filter( function(d) {
      return d.source.geo !== null && d.target.geo !== null && d.type !== "vpn" && d.type !== "backbone"
    })

    var backbones = data.links.filter( function(d) {
      return d.source.geo !== null && d.target.geo !== null && d.type === "backbone"
    })
    ...
    var backboneSvg = g.select(".backbones").selectAll(".backbone").data(backbones, function(d) { return d.id })
    ...
    backboneSvg.enter().append("line")
            .attr("class", "backbone")

    backboneSvg.style("stroke", function(d) {
          switch (d.type) {
            case "vpn":
              return linkColor.default(Math.max.apply(null, d.quality.split(",")))
            default:
    ...
    // Set initial view focused on Mainz City
    map.setView(new L.LatLng(49.993652247452, 8.251419067382812), 12)
    ...
    function reset() {
      ...
      var backbonesSvg = g.selectAll(".backbone")
      ...
      backbonesSvg.attr("x1", function (d) { return project(d.source.geo)[0] })
               .attr("y1", function (d) { return project(d.source.geo)[1] })
               .attr("x2", function (d) { return project(d.target.geo)[0] })
               .attr("y2", function (d) { return project(d.target.geo)[1] })
