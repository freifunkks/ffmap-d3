define("graph", [
  ...
  meshinfo.append("p")
          .attr("id", "allnodecount")

  meshinfo.append("p")
          .attr("id", "nodecount")

  meshinfo.append("p")
          .attr("id", "backbonecount")
  ...
  function showNodeInfo(d) {
    ...
    nodeinfo.append("h2").text(d.name + " / " + d.id)

    if (d.group === "gateway") {
      nodeinfo.append("p")
                .append("label")
                .text("Freifunk Gateway")

      if (d.hardware) {
        nodeinfo.append("h2").text("Hardware")

        nodeinfo.append("p")
                  .append("label")
                  .text("Plattform: " + d.hardware)
      }
      ...
      if (d.firmware) {
        ...
        nodeinfo.append("p")
                .append("label")
                .text("Version: " + d.firmware)
      }

      if (d.batman_version) {
        nodeinfo.append("h2")
                .text("B.A.T.M.A.N. Advanced")

        nodeinfo.append("p")
                .append("label")
                .text("Version: " + d.batman_version)

        if (d.batman_gwmode === "server")
          nodeinfo.append("p")
                  .append("label")
                  .text("Gatewayfunktion: aktiviert")
        else
          nodeinfo.append("p")
                  .append("label")
                  .text("Gatewayfunktion: deaktiviert")
      }
    } else if (d.group === "service") {
      nodeinfo.append("p")
                .append("label")
                .text("Freifunk Service")
      ...
      if (d.group === "backbone")
        nodeinfo.append("p")
                  .append("label")
                  .text("Freifunk Backbone")
      else
        nodeinfo.append("p")
                  .append("label")
                  .text("Freifunk Knoten")
      ...
      if (d.hardware) {
        ...
        nodeinfo.append("p")
                  .append("label")
                  .text("Modell: " + d.hardware)
      }
      if (d.addresses) {
        nodeinfo.append("h2").text("IP Adressen:")

        d.addresses.forEach(function(ip) {
          nodeinfo.append("p")
                  .append("label")
                  .text(ip)
        })
      ...
      if (d.firmware)
        if (data.meta.gluon_release > d.firmware) {
          nodeinfo.append("h2")
                  .text("Firmware (veraltet)")

          nodeinfo.append("p")
                  .append("label")
                  .text(d.autoupdater_state ? "Autoupdates: aktiviert" : "Autoupdates: deaktiviert")

          nodeinfo.append("p")
                  .append("label")
                  .text("Branch: " + d.autoupdater_branch)
      ...
      nodeinfo.append("h2").text("Statistiken")
  ...
  var force = d3.layout.force()
                ...
                .linkStrength(function (d) {
                  switch (d.type) {
                    ...
                    case "backbone": return 0.01 * strengthScale
  ...
  function reload(json) {
    ...
    if ("meta" in data) {
      ...
      updatedAt.text("Stand: " + date)
    }

    var nNodes = data.nodes.filter(function(d) {
                   return d.group !== "gateway" && d.group !== "service" && d.group !== "backbone" && d.flags.online
                 }).length,
        nAllNodes = data.nodes.filter(function(d) {
                   return d.group !== "gateway" && d.group !== "service" && d.group !== "backbone"
                 }).length,
        nServices = data.nodes.filter(function(d) {
                   return d.group === "service" && d.flags.online
                 }).length,
        nBackbones = data.nodes.filter(function(d) {
                   return d.group === "backbone" && d.flags.online
                 }).length,
    ...
    d3.select("#allnodecount")
      .text(nAllNodes + " Knoten gesamt")

    d3.select("#nodecount")
      .text("davon " + nAllNodes + " Online")

    d3.select("#backbonecount")
      .text(nBackbones + " Backbone Knoten")

    d3.select("#servicecount")
      .text(nServices + " Service Knoten")
  ...
  function update() {
    ...
    node.attr("class", function(d) {
          var s = ["node"]
          if (d.group !== "gateway" && d.group !== "service")
            if (data.meta.gluon_release > d.firmware)
              s.push(["legacy"])
